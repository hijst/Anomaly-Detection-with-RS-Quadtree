...
forest_cover_raw = pd.read_pickle('../data/forest_c.pkl')
forest_cover = normalize(forest_cover_raw)
forest_cover_list = forest_cover.values.tolist()
forest_cover_data = forest_cover.iloc[:, :10]
forest_cover_answers = forest_cover.iloc[:, 10:]
forest_cover_labels = []
for fc in forest_cover_answers[54]:
    if fc == 0.0:
        forest_cover_labels.append(1)
    else:
        forest_cover_labels.append(-1)
# print(forest_cover_labels)
...
def main(plot_it=False):
    for j in ct:
        st = time.time()
        results_if = IsolationForest(contamination=j, n_estimators=100).fit_predict(forest_cover_data)
        ctr = 0
        ctr2 = 0
        ctr3 = 0
        for ind, res in enumerate(results_if):
            if forest_cover_labels[ind] == -1 and res == -1:
                ctr += 1
            if res == -1:
                ctr2 += 1
        print("Contamination: ", j)
        print("True Positives: ", ctr, "percentage correct: ", ctr / int(j * len(forest_cover_labels)))
        print("Total Positives: ", ctr2)
        print("ROC-AUC Score: ", roc_auc_score(forest_cover_labels, results_if))
        print("results if: ", results_if)
        print("Running time: ", time.time()-st)

    start_time = time.time()
    clf = RSForest(contamination=ct2, k=15, points=forest_cover_list, granularity=5, sample_size=256)

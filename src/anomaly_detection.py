...
gaussian = make_blobs(centers=[[0, 0]], n_features=2, random_state=0, n_samples=[n_inliers])[0]
gaussian_mix = make_blobs(centers=[[0, 0], [5, 5]], n_features=2, random_state=0,
                          n_samples=[462, 462], cluster_std=[1.5, 0.5])[0]
...
for i_dataset, X in enumerate(datasets):
    ...
    for name, algorithm in anomaly_algorithms:
        ...
        data_names = ["Gaussian", "Small cluster", "Mixed Gaussians", "Circle Blob", "Circle Blob XL"]
        if name == "Robust covariance":
            plt.ylabel(data_names[i_dataset], fontsize=16)
...
#plt.savefig('../output/gaussians_mix_comparisons2.pdf')
